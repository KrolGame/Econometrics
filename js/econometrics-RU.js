...
function calc(){
	var i;
	//Получение выбранной регрессии через селект
	var selectValue = document.getElementById('select').value;

	//Получение выбранной степени округления чисел через селект
	...
	var arry = fun_arry();	
	var n = arrx.length; //Запись количества наблюдений
	var bool = fun_check();
	...
	//Сумма всех значений из массива иксов
	...
	//Сумма всех значений из массива игреков
	...
	//Записываем массив значений x^2
	...
	//Сумма всех значений массива x^2
	...
	//Записываем массив значений x^3
	...
	//Сумма всех значений массива x^3
	...
	//Записываем массив значений x^4
	...
	//Сумма всех значений массива x^4
	...
	//Сумма всех значения массива Xi * Yi
	...
	//Сумма всех значений массива Xi^2 * Yi
	...
	//Сумма всех значений массива log(x)
	...
	//Сумма всех значений массива log(y)
	...
	//Сумма всех значений массива log(x)^2
	...
	//Сумма всех значений массива lnXi * lnYi
	...
	//Сумма всех значений массива Xi * lnYi
	...
	//Сумма всех значений массива lnXi * Yi
	...
	//Сумма всех значений массива 1 / x
	...
	//Сумма всех значений массива 1 / x^2
	...
	//Сумма всех значений массива y / x
	...
	if(bool == 0){
		...
		//Расчет значений для линейной регрессии
		if(selectValue == 0){
		...
		//Расчет значений для степенной регрессии
		if(selectValue == 1){
			arrlnx = fun_arrlnx();
		...
		//Расчет значений для параболической регрессии
		if(selectValue == 2){
			arr_pow2x = fun_arr_pow2x();
			sum_pow2x = parseFloat((fun_sum_arr_pow2x()).toFixed(selectRound));
		...
		//Расчет значений для гиперболической регрессии
		if(selectValue == 3){
		...
		//Расчет значений для экспоненциальной и показательной регрессий
		...
		//Расчет значений для логарифмической регрессии
		if(selectValue == 5){
			arrlnx = fun_arrlnx();
			sumlnx = parseFloat((fun_sum_arrlnx()).toFixed(selectRound));
			arr_powlnx = fun_arr_powlnx();
		...
		//Нахождение a и b для линейной регрессии
		if(selectValue == 0){
		...
		//Нахождение a и b для степенной регрессии
		if(selectValue == 1){
		...
		//Нахождение a, b, c для параболической регрессии
		if(selectValue == 2){
		...
		//Нахождение a и b для гиперболической регрессии
		if(selectValue == 3){
		...
		//Нахождение a и b для экспоненциальной регрессии
		if(selectValue == 4){
		...
		//Нахождение a и b для логарифмической регрессии
		if(selectValue == 5){
		...
		//Нахождение a и b для показательной регрессии
		if(selectValue == 6){
			b = parseFloat(((sum_multi_x_lny - (sumx * sumlny / n)) / (sum_pow2x - (sumx * sumx / n))).toFixed(selectRound));
			a = parseFloat(((sumlny - (sumx * b)) / n).toFixed(selectRound));
			a_exp = parseFloat((Math.exp(a)).toFixed(selectRound));
		...
		//Расчет и запись массива значений Ymod для линейной регрессии
		if(selectValue == 0){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для степенной регрессии
		if(selectValue == 1){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для параболической регрессии
		if(selectValue == 2){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для гиперболической регрессии
		if(selectValue == 3){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для экспоненциальной регрессии
		if(selectValue == 4){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для логарифмической регрессии
		if(selectValue == 5){
			for(i=0; i<n; i++){
		...
		//Расчет и запись массива значений Ymod для показательной регрессии
		if(selectValue == 6){
			for(i=0; i<n; i++){
		...
		//Расчет корреляции для линейной регрессии
		if(selectValue == 0){
		...
		//Расчет корреляции для остальных видов регрессий
		...
		//Расчет коэффициента(индекса) детерминации и влияние Y на X для любого вида регрессии
		...
		//Расчет критерия Фишера для всех видов регрессий кроме параболистической
		...
		//Расчет критерия Фишера для параболической регрессии
		if(selectValue == 2){
		...
		//Расчет ошибки аппроксимации для любого вида регрессии
		var err_apr = 0;
			for(i=0; i<n; i++){
		...
		var w_eqab = "";
			...
			if(selectValue == 2){w_eqab = '<h3>Для нахождения a, b, c - воспользуемся методом Крамера:<br><br>Для начала сосчитаем знаменатель Крамера(D), для этого построим матрицу 1:</h3><h4>Матрица 1</h4><table class="generate-table"><tbody><tr><td>'+n+'</td><td>'+sumx+'</td><td>'+sum_pow2x+'</td><td>'+n+'</td><td>'+sumx+'</td></tr><tr><td>'+sumx+'</td><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td><td>'+sumx+'</td><td>'+sum_pow2x+'</td></tr><tr><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td><td>'+sumpow4x+'</td><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td></tr></tbody></table><h3>D = ('+n+' * '+sum_pow2x+' * '+sumpow4x+') + ('+sumx+' * '+sumpow3x+' * '+sum_pow2x+') + ('+sum_pow2x+' * '+sumx+' * '+sumpow3x+') - ('+sum_pow2x+' * '+sum_pow2x+' * '+sum_pow2x+') - ('+sumpow3x+' * '+sumpow3x+' * '+n+') - ('+sumpow4x+' * '+sumx+' * '+sumx+') = '+d+'</h3><h3>Теперь сосчитаем 1 числитель Крамера(Da), для этого построим матрицу 2:</h3><h4>Матрица 2</h4><table class="generate-table"><tbody><tr><td>'+sumy+'</td><td>'+sumx+'</td><td>'+sum_pow2x+'</td><td>'+sumy+'</td><td>'+sumx+'</td></tr><tr><td>'+sum_multi_x_y+'</td><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td><td>'+sum_multi_x_y+'</td><td>'+sum_pow2x+'</td></tr><tr><td>'+sum_multi_pow2x_y+'</td><td>'+sumpow3x+'</td><td>'+sumpow4x+'</td><td>'+sum_multi_pow2x_y+'</td><td>'+sumpow3x+'</td></tr></tbody></table><h3>Da = ('+sumy+' * '+sum_pow2x+' * '+sumpow4x+') + ('+sumx+' * '+sumpow3x+' * '+sum_multi_pow2x_y+') + ('+sum_pow2x+' * '+sum_multi_x_y+' * '+sumpow3x+') - ('+sum_multi_pow2x_y+' * '+sum_pow2x+' * '+sum_pow2x+') - ('+sumpow3x+' * '+sumpow3x+' * '+sumy+') - ('+sumpow4x+' * '+sum_multi_x_y+' * '+sumx+') = '+da+'</h3><h3>Теперь сосчитаем 2 числитель Крамера(Db), для этого построим матрицу 3:</h3><h4>Матрица 3</h4><table class="generate-table"><tbody><tr><td>'+n+'</td><td>'+sumy+'</td><td>'+sum_pow2x+'</td><td>'+n+'</td><td>'+sumy+'</td></tr><tr><td>'+sumx+'</td><td>'+sum_multi_x_y+'</td><td>'+sumpow3x+'</td><td>'+sumx+'</td><td>'+sum_multi_x_y+'</td></tr><tr><td>'+sum_pow2x+'</td><td>'+sum_multi_pow2x_y+'</td><td>'+sumpow4x+'</td><td>'+sum_pow2x+'</td><td>'+sum_multi_pow2x_y+'</td></tr></tbody></table><h3>Db = ('+n+' * '+sum_multi_x_y+' * '+sumpow4x+') + ('+sumy+' * '+sumpow3x+' * '+sum_pow2x+') + ('+sum_pow2x+' * '+sumx+' * '+sum_multi_pow2x_y+') - ('+sum_pow2x+' * '+sum_multi_x_y+' * '+sum_pow2x+') - ('+sum_multi_pow2x_y+' * '+sumpow3x+' * '+n+') - ('+sumpow4x+' * '+sumx+' * '+sumy+') = '+db+'</h3><h3>Теперь сосчитаем 3 числитель Крамера(Dc), для этого построим матрицу 4:</h3> <h4>Матрица 4</h4><table class="generate-table"><tbody><tr><td>'+n+'</td><td>'+sumx+'</td><td>'+sumy+'</td><td>'+n+'</td><td>'+sumx+'</td></tr><tr><td>'+sumx+'</td><td>'+sum_pow2x+'</td><td>'+sum_multi_x_y+'</td><td>'+sumx+'</td><td>'+sum_pow2x+'</td></tr><tr><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td><td>'+sum_multi_pow2x_y+'</td><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td></tr></tbody></table><h3>Dc = ('+n+' * '+sum_pow2x+' * '+sum_multi_pow2x_y+') + ('+sumx+' * '+sum_multi_x_y+' * '+sum_pow2x+') + ('+sumy+' * '+sumx+' * '+sumpow3x+') - ('+sum_pow2x+' * '+sum_pow2x+' * '+sumy+') - ('+sumpow3x+' * '+sum_multi_x_y+' * '+n+') - ('+sum_multi_pow2x_y+' * '+sumx+' * '+sumx+') = '+dc+'</h3><h3>Теперь сосчитаем значения a, b, c:<br><br>a = Da / D = '+a+';<br>b = Db / D = '+b+';<br>c = Dc / D = '+c+';</h3>';}
		var w_eqymod = "";
		...
		//Генерация 1 блока
		gdiv = ['<h2>Решение</h2><h3>Уравнение регрессии имеет вид:<br><br>Y = '+w_eq+'</h3><h3>Сосчитаем параметры уравнения через систему линейных уравнений<br><br>Система линейных уравнений для нахождения параметров уравнения:<br><br>'+w_sleq+'</h3><h3>Для решения системы уравнений построим и сосчитаем значения таблицы 1:</h3>'];
		document.getElementById('generate1').innerHTML=gdiv.join('\n');

		//Генерация 3 блока
		gdiv = ['<h3>Теперь система уравнений будет иметь следующий вид:<br><br>'+w_eq2+'</h3>'+w_eqab+'<h3>Уравнение регрессии будет иметь следующий вид:<br><br>Ymod = '+w_eqymod+'</h3><h3>Для дальнейшего решения и оценки качества модели построим и сосчитаем значения таблицы 2:</h3>'];
		document.getElementById('generate3').innerHTML=gdiv.join('\n');

		//Генерация 2, 4 и 5 блоков для линейной регрессии
		if(selectValue == 0){
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumx+'</td><td>'+sumy+'</td><td>'+sum_pow2x+'</td><td>'+sum_multi_x_y+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');

			gtable = ['<table class="generate-table"><h4>Таблица 2<h4>'];
			gtable.push('<tr><td></td><td>Y</td><td>Ymod</td><td>(Yсред - Ymod)^2</td><td>(Y - Yсред)^2</td></tr>');
				for (i=0; i<n; i++){
					gtable.push('<tr><td>'+(i + 1)+'</td><td>'+arry[i]+'</td><td>'+arrmody[i]+'</td><td>'+arr_midy_minus_mody_pow[i]+'</td><td>'+arr_y_minus_midy_pow[i]+'</td></tr>');
				}
			gtable.push('<tr><td>Сумма</td><td>'+sumy+'</td><td></td><td>'+sum_midy_minus_mody_pow+'</td><td>'+sum_y_minus_midy_pow+'</td></tr>');
			gtable.push('<tr><td>Среднее</td><td>'+midy+'</td><td></td><td></td><td></td></tr></table>');
			document.getElementById('generate4').innerHTML=gtable.join('\n');

			gdiv = ['<h3>Сосчитаем коэффициент корреляции по формуле:<br><br>R = &radic;<span> ∑((Yсред - Ymod)^2) / ∑((Y - Yсред)^2)</span><br><br>R = &radic;<span> '+sum_midy_minus_mody_pow+' / '+sum_y_minus_midy_pow+'</span> = '+correl+'</h3><h3>Для оценки связи между X и Y воспользуемся таблицей 3:</h3>'];
			document.getElementById('generate5').innerHTML=gdiv.join('\n');
		}

		//Генерация 2 блока для степенной регрессии
		if(selectValue == 1){
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumlnx+'</td><td>'+sumlny+'</td><td>'+sumpowlnx+'</td><td>'+sum_multi_lnx_lny+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');
		}

		//Генерация 2 блока для параболической регрессии
		if(selectValue == 2){			
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumx+'</td><td>'+sumy+'</td><td>'+sum_pow2x+'</td><td>'+sumpow3x+'</td><td>'+sumpow4x+'</td><td>'+sum_multi_x_y+'</td><td>'+sum_multi_pow2x_y+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');
		}

		//Генерация 2 блока для гиперболической регрессии
		if(selectValue == 3){		
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumy+'</td><td>'+sum_divis_x+'</td><td>'+sum_divis_pow2x+'</td><td>'+sum_divis_y_x+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');
		}

		//Генерация 2 блока для экспоненциальной регрессии
		if(selectValue == 4){
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumx+'</td><td>'+sumlny+'</td><td>'+sum_pow2x+'</td><td>'+sum_multi_x_lny+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');
		}

		//Генерация 2 блока для логарифмической регрессии
		if(selectValue == 5){			
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			...
			gtable.push('<tr><td>Сумма</td><td>'+sumy+'</td><td>'+sumlnx+'</td><td>'+sum_powlnx+'</td><td>'+sum_multi_lnx_y+'</td></tr></table>');
			document.getElementById('generate2').innerHTML=gtable.join('\n');
		}

		//Генерация 2 блока для показательной регрессии
		if(selectValue == 6){
			gtable = ['<table class="generate-table"><h4>Таблица 1<h4>'];
			gtable.push('<tr><td></td><td>X</td><td>lnY</td><td>X^2</td><td>X * lnY</td></tr>');
				for (i=0; i<n; i++){
		...
		//Генерация 4 и 5 блоков для всех видов регрессии кроме линейной
		if(selectValue == 1 || selectValue == 2 || selectValue == 3 || selectValue == 4 || selectValue == 5 || selectValue == 6){
			gtable = ['<table class="generate-table"><h4>Таблица 2<h4>'];
			gtable.push('<tr><td></td><td>Y</td><td>Ymod</td><td>(Y-Ymod)^2</td><td>(Y - Yсред)^2</td></tr>');
				for (i=0; i<n; i++){
					gtable.push('<tr><td>'+(i + 1)+'</td><td>'+arry[i]+'</td><td>'+arrmody[i]+'</td><td>'+arr_y_minus_mody_pow[i]+'</td><td>'+arr_y_minus_midy_pow[i]+'</td></tr> ');
				}
			gtable.push('<tr><td>Сумма</td><td>'+sumy+'</td><td></td><td>'+sum_y_minus_mody_pow+'</td><td>'+sum_y_minus_midy_pow+'</td></tr>');
			gtable.push('<tr><td>Среднее</td><td>'+midy+'</td><td></td><td></td><td></td></tr></table>');
			document.getElementById('generate4').innerHTML=gtable.join('\n');
			
			gdiv = ['<h3>Сосчитаем индекс корреляции по формуле:<br><br>R = &radic;<span> 1 - (∑(Y - Ymod)^2) / ∑(Y - Yсред)^2))</span><br><br>R = &radic;<span> 1 - ('+sum_y_minus_mody_pow+' / '+sum_y_minus_midy_pow+')</span> = '+correl+'</h3><h3>Для оценки связи между X и Y воспользуемся таблицей 3:</h3>'];
			document.getElementById('generate5').innerHTML=gdiv.join('\n');
		}		

		//Общая генерация
		gtable = ['<table class="generate-table"><h4>Таблица 3<h4><tr><td>R = 0</td><td>Связь отсутствует</td></tr><tr><td>0 < R < 0.2</td><td>Очень слабая связь</td></tr><tr><td>0.2 < R < 0.3</td><td>Слабая связь</td></tr><tr><td>0.3 < R < 0.5</td><td>Умеренная связь</td></tr><tr><td>0.5 < R < 0.7</td><td>Средняя связь</td></tr><tr><td>0.7 < R < 0.9</td><td>Сильная связь</td></tr><tr><td>0.9 < R < 1</td><td>Очень сильная связь</td></tr><tr><td>R = 1</td><td>Функциональная связь</td></tr></table>'];
		document.getElementById('generate6').innerHTML=gtable.join('\n');

		gdiv = ['<div>'];
		if(correl == 0){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y отсутствует</h3>');
		}
		if(correl > 0 && correl < 0.2){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y очень слабая</h3>');
		}
		if(correl >= 0.2 && correl < 0.3){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y слабая</h3>');
		}
		if(correl >= 0.3 && correl < 0.5){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y умеренная</h3>');
		}
		if(correl >= 0.5 && correl < 0.7){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y средняя</h3>');
		}
		if(correl >= 0.7 && correl < 0.9){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y высокая</h3>');
		}
		if(correl >= 0.9 && correl < 1){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y очень высокая</h3>');
		}
		if(correl == 1){
			gdiv.push('<h3>'+w_ic+' корреляции = '+correl+' - Связь между X и Y функциональная</h3>');
		}
		gdiv.push('</div>');
		document.getElementById('generate7').innerHTML=gdiv.join('\n');

		gdiv = ['<h3>Сосчитаем '+w_ic+' детерминации:<br><br>Так как '+w_ic+' детерминации это '+w_ic+' корреляции во 2 степени, то:<br><br>R^2 = '+deter+'</h3><h3>'+w_ic+' детерминации = '+deter+' - На '+deter_100+'% вариация Y обусловлена вариацией X; '+deter_other+'% вариация прочих факторов</h3>'];
		document.getElementById('generate8').innerHTML=gdiv.join('\n');

		gdiv = ['<h3>Сосчитаем критерий Фишера по формуле:<br><br>F = (R^2 / (1 - R^2)) * (n - '+w_f+')<br><br>F = ('+deter+' / (1 - '+deter+')) * ('+n+' - '+w_f+') = '+fisher+'<br><br>Табличное значение Фишера = '+fisher_tabl+'</h3>'];
		document.getElementById('generate9').innerHTML=gdiv.join('\n');

		gdiv = ['<div>'];
		if(fisher > fisher_tabl){
			gdiv.push('<h3>Так как '+fisher+' > '+fisher_tabl+', то '+w_ic+' детерминации статистически значим и уравнение регрессии значимо в целом</h3>');
		}
		if(fisher < fisher_tabl){
			gdiv.push('<h3>Так как '+fisher+' < '+fisher_tabl+', то '+w_ic+' детерминации статистически не значим и уравнение регрессии не значимо в целом</h3>');
		}
		gdiv.push('</div>');
		document.getElementById('generate10').innerHTML=gdiv.join('\n');

		gdiv = ['<div>'];
		gdiv.push('<h3>Сосчитаем ошибку аппроксимации по формуле:<br><br>A% = (1 / n) * ∑((Y - Ymod) / Y) * 100<br><br>A% = '+err_apr+'%</h3>');
		if(err_apr < 7){
			gdiv.push('<h3>Так как A% = '+err_apr+' < 7%, то модель является качественной</h3>');
		}
		if(err_apr == 7 && err_apr <= 10){
			gdiv.push('<h3>Так как A% = '+err_apr+' и находится в пределе от 7% до 10%, то модель является приемлемой</h3>');
		}
		if(err_apr > 7){
			gdiv.push('<h3>Так как A% = '+err_apr+' > 10%, то модель является некачественной и данное уравнение регрессии не желательно использовать</h3>');
		}
		gdiv.push('</div>');
