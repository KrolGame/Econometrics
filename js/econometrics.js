...
function calc(){
	...
	if(bool == 0){
		...
		if(selectValue == 0){
			...
			correl = parseFloat((Math.abs((sum_midy_minus_mody_pow / sum_y_minus_midy_pow))**(1/2)).toFixed(selectRound));
		}
		...
		if(selectValue == 1 || selectValue == 2 || selectValue == 3 || selectValue == 4 || selectValue == 5 || selectValue == 6){
			...
			sum_y_minus_midy_pow = parseFloat(sum_y_minus_midy_pow.toFixed(selectRound));
			correl = parseFloat((Math.abs((1 - (sum_y_minus_mody_pow / sum_y_minus_midy_pow)))**(1/2)).toFixed(selectRound));
		}	
		...
		//Генерация 1 блока
		gdiv = ['<h2>Решение</h2><h3>Уравнение регрессии имеет вид:<br><br>Y = '+w_eq+'</h3><h3>Сосчитаем параметры уравнения a и b через систему линейных уравнений<br><br>Система линейных уравнений для нахождения значений a и b:<br><br>'+w_sleq+'</h3><h3>Для решения системы уравнений построим и сосчитаем значения таблицы 1:</h3>'];
		...
		if(err_apr > 7){
			gdiv.push('<h3>Так как A% = '+err_apr+' > 10%, то модель является не качественной и данное уравнение регрессии не желательно использовать</h3>');
		}
		gdiv.push('</div>');
